...
const services = [
  ...
  {
    title: "Массаж при перекосах в теле",
    text: "На этот массаж Вы можете записаться, если хотите узнать, есть ли у Вас перекос, а также при перекосе плечевого пояса и тазового отдела, сутулости, сколиозе, гиперлордозе, кифозе и других асимметриях в теле.",
    durations: {
      moscow: [60, 90, 120],
      saratov: [60, 90, 120]
    }
...
function updatePromoVisibility() {
  const promoSection = document.getElementById("moscowPromoUntilJuly");
  if (!promoSection) return;

  promoSection.hidden = (currentCity || "moscow") !== "moscow";
}
